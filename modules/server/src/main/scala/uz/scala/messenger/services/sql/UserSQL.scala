...
object UserSQL extends SQLTable {
  ...
  private val columns = userId ~ username ~ tel ~ zonedDateTime ~ status ~ passwordHash

  val encoder: Encoder[UserId ~ ZonedDateTime ~ CreateUser ~ PasswordHash[SCrypt]] =
    columns.contramap {
      case i ~ cr ~ u ~ p =>
        i ~ u.username ~ u.phone ~ cr ~ Offline ~ p
    }
  ...
  val common: Decoder[UserWithPassword] =
    columns.map {
      case i ~ n ~ t ~ cr ~ s ~ p =>
        UserWithPassword(User(i, n, t, cr, s), p)
    }

  val selectByPhone: Query[Tel, UserWithPassword] =
    sql"""SELECT * FROM users WHERE phone = $tel""".query(common)

  val insert: Query[UserId ~ ZonedDateTime ~ CreateUser ~ PasswordHash[SCrypt], User] =
