...
final class UserRoutes[F[_]: Async](userService: UserService[F])(implicit
    ...
    case req @ POST -> Root / "login" =>
      authService
        ...
        .recoverWith {
          ...
          case error =>
            logger.error(s"Error occurred while authorization. Error: $error") >>
              BadRequest("Email or password isn't correct!")
        }
  ...
  private[this] val privateRoutes: HttpRoutes[F] = authService.securedRoutes {
    ...
    case GET -> Root / "all" asAuthed user =>
      userService.getAll.map(_.filterNot(_.id == user.id)).flatMap(Ok(_))
